...
- Adjacent corridors merge into one walkable space in-game, which silently strips any door between two corridor pieces on load (v5 re-export: all 10 corridor-corridor doors dropped, 18 others kept). The generator never emits them; connectivity and pathfinding treat touching corridor pieces as one space; the door tree spans merged corridor blobs.
...
- Placement rules come from the game's own part rules files (rules.7z, extracted locally, never committed), not from fitting alone. A programmatic diff of all 100+ parts against `parts-db.ts` found every AllowedDoorLocations table already matched. Corrections taken from the source of truth: IsRotateable flags now match the files exactly (85 rotateable; storage_2x2 and reactors are fixed, weapons/thrusters are game-rotateable but stay at rot 0 by generator policy via `generatorRotsFor`), and each part carries its ProhibitLeft/Right/Above/Below depths in `prohibit`. Thruster exhaust reservation uses the per-part ProhibitBelow (small 3, med 5, large 7, huge 10, boost 18) instead of a flat 3; weapon firing-arc reservation keeps the design rule of a clear column north (depth 64) which subsumes the game's 1-2 cell barrel clearance (`ProhibitAbove`).
