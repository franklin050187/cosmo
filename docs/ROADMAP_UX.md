...
| 4.3 | **Delete confirmation dialog** (currently captcha-as-confirmation with no confirm copy). | `games/[id]/page.tsx` | S |
| 4.4 | **Bracket**: connector lines between rounds; editable matchups / re-seed / manual slot; score/set tracking; explain byes & advancement; `aria-live` on winner/champion. | `Bracket.tsx`, games API | L |
...
| 4.11 | **IA collision**: `/game` (About Cosmoteer) vs `/games` (community games) — rename `/game` URL and add header link. | `game/page.tsx`, `Header.tsx`, `Footer.tsx` | S |
