...
| 4.4 | **Bracket** — done: aligned grid + SVG connector lines between rounds, winner-path highlight, **winner-drop lines** (WB loser → LB target, lit when decided), **"BYE" labels** on decided bye slots, double elim (winners top / losers under / GF rightmost), ✓ gated until both feeding matches are decided, losers-R1 singleton auto-advance, explain-byes legend + `aria-live` announcements. **Round robin added**: circle-method schedule, standings table with head-to-head tiebreak, per-round match cards, migration 009 (`rr` bracket). Remaining: editable matchups / re-seed; score/set tracking. | `Bracket.tsx`, `RoundRobinView.tsx`, games API | L | 🚧 |
