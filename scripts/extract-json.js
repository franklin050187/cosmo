...
// Copy the decode implementation from decode-ship.mjs
...
async function extractShipPngToJson(filePath, outputPath) {
  try {
    ...
    console.log(`Extracted ${path.basename(outputPath)} to JSON`);
    return value;
  } catch (err) {
    console.error(`Failed to extract ${path.basename(filePath)}:`, err.message);
    return null;
  }
}

async function main() {
  const inputDir = './scripts/price-test';
  const outputDir = './scripts/price-test/extracted-json';
  ...
  for (const file of files) {
    ...
    const outputPath = path.join(outputDir, file.replace('.png', '.json'));
    await extractShipPngToJson(inputPath, outputPath);
  }
}
